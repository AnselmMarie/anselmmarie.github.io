...
**Status:** not started · **Visible?** ✅ screen · **Depends on:** nothing
...
[Q10](../questions-closed.md#q10) blocked this slice and closed on 2026-09-20 as
...
- **[Q11](../questions-closed.md#q11) → [D44](../decisions-d42-d47.md#d44)** — ESLint owns the
...
- **[Q16](../questions-closed.md#q16) → [D45](../decisions-d42-d47.md#d45)** — `.nvmrc` pins Node
  22 and `.npmrc` was added.
- **[Q15](../questions-closed.md#q15) → [D46](../decisions-d42-d47.md#d46)** — the half-ported
  cosmikata tooling was pruned. What that leaves this slice is the repaired-state note below.

⚠️ **[Q14](../open-questions.md#q14) is the one still open**, and it is the one that matters
here. It asks whether `apps/shell` emits per-route metadata — `<title>`, description, Open
Graph — server-side from `libs/shared/fixtures`, even though
[D36](../decisions-d33-d41.md#d36) leaves the visible content client-rendered. It does not
change what this slice builds, but it decides whether the shell's route files gain a
metadata source, so settle it before the route tree is written.
...
[Q15](../questions-closed.md#q15) closed the same day as [D46](../decisions-d42-d47.md#d46): the
...
- `.gitignore` — present, but still carries Next.js, Expo, Cloudflare Wrangler, OpenNext and
  Storybook sections that are all dead here ([D6](../decisions-d01-d16.md#d6),
  [D31](../decisions-d17-d32.md#d31)). Prune those, then extend for Nx, Vite, and
  build/deploy output.

**`apps/shell`**

- `project.json`, `vite.config.ts`, the Lambda adapter/handler config, `tailwind.config.ts`
...
```bash
pnpm nx run-many -t typecheck lint test --projects=shell,feature-shell,shared-types,shared-config,shared-fixtures,shared-utils
```

Plus the visible check: the page open in a browser, and a screenshot in the completion
report ([plan-visible-first.md](../../../../.claude/rules/plan-visible-first.md)).
