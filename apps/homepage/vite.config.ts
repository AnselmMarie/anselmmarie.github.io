...
/**
 * The Homepage remote's build (D1 — a standalone React + Vite app, not a
 ...
 * inherits them. Slice 6 owns what this remote *renders*; the wiring here is
 ...
 * `DEFAULT_HOMEPAGE_ORIGIN` in `@portfolio/shared-config`. */
const DEV_PORT = 4176;
...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    ...
    federation({
      ...
      // D27 — the remote's public surface, in one line. `./Homepage` is the
      // module specifier the shell's registry entry names.
      exposes: { './Homepage': '@portfolio/feature-homepage' },
